...
//    }
    ...
    outerTriangle.push_back(ApolloniusGraph_Site2(Site2_Point2(v.x,v.y-rcst),MAX_WEIGHT));
    outerTriangle.push_back(ApolloniusGraph_Site2(Site2_Point2(v.x - rcot, v.y + r),MAX_WEIGHT));
    outerTriangle.push_back(ApolloniusGraph_Site2(Site2_Point2(v.x + rcot, v.y + r),MAX_WEIGHT));
    for(auto v: outerTriangle){
        diagram.add_node_cgal(v);
    }
    
    for(int i = 0; i < initial_nodes; ++i){
        diagram.add_node(ofRandom(ofGetWidth()), ofRandom(ofGetHeight()), ofRandom(MAX_WEIGHT)-10);
    }
    
    Iso_rectangle_2 bbox(0,0,2000,2000);
    Cropped_voronoi_from_delaunay vor(bbox);
    
//    diagram.voronoi.dual();
}
...
void ofApp::draw(){
        ...
        for(int i = 0; i < diagram.get_n_nodes(); i++){
            
            // pick up point
            Point_2 p(diagram.nodes[i].x(), diagram.nodes[i].y());
